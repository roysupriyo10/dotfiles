#!/usr/bin/env bash
# Antigravity (agy) status line — session token usage
set -euo pipefail
...
width="${COLUMNS:-}"; if [[ -z "$width" ]]; then width=$(tput cols < /dev/tty 2>/dev/null || echo 200); fi
