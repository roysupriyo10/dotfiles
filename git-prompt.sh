...
__git_ps1_show_upstream ()
{
	...
	if [ -z "$verbose" ]; then
		...
		*)	    # diverged from upstream
		...
		local _up_r="${__git_SOH}${__git_ESC}[31m${__git_STX}"
		local _up_g="${__git_SOH}${__git_ESC}[32m${__git_STX}"
		local _up_n="${__git_SOH}${__git_ESC}[0m${__git_STX}"
		case "$count" in
		"") # no upstream
			upstream="" ;;
		"0	0") # equal to upstream
			upstream="" ;;
		"0	"*) # ahead of upstream
			upstream=" ${_up_g}>${count#0	}${_up_n}" ;;
		*"	0") # behind upstream
			upstream=" ${_up_r}<${count%	0}${_up_n}" ;;
		*)	    # diverged from upstream
			upstream=" ${_up_r}<${count%	*}${_up_n} ${_up_g}>${count#*	}${_up_n}" ;;
		esac
...
__git_ps1_colorize_gitstring ()
{
	...
	if [ -n "$r" ]; then
		r=" $bad_color${r# }$c_clear"
	fi
	if [ -n "$conflict" ]; then
		conflict=" $bad_color${conflict# }$c_clear"
	fi
}
...
__git_ps1 ()
{
	...
	# Treat rebase/am as detached (HEAD is mid-application)
	case "$r" in
	*rebase*|*am*) detached=yes ;;
	esac

	# Any detached state (rebase, am, plain detached HEAD, submodules) → @<sha>
	if [ "$detached" = yes ] && [ -n "$short_sha" ]; then
		b="@$short_sha"
	fi

	local conflict="" # state indicator for unresolved conflicts
	local _conflict_count
	_conflict_count=$(git ls-files --unmerged 2>/dev/null | awk '{print $4}' | sort -u | wc -l | tr -d ' ')
	if [ -n "$_conflict_count" ] && [ "$_conflict_count" -gt 0 ]; then
		conflict=" ~$_conflict_count"
	fi
	...
	if [ "true" = "$inside_gitdir" ]; then
		...
		then
			if ! git diff --no-ext-diff --quiet; then
				local _w_count
				_w_count=$(git diff --no-ext-diff --name-only 2>/dev/null | wc -l | tr -d ' ')
				w="*$_w_count"
			fi
			if ! git diff --no-ext-diff --cached --quiet; then
				local _i_count
				_i_count=$(git diff --no-ext-diff --cached --name-only 2>/dev/null | wc -l | tr -d ' ')
				i="+$_i_count"
			fi
		...
		then
			local _u_count
			_u_count=$(git ls-files --others --exclude-standard 2>/dev/null | wc -l | tr -d ' ')
			if [ "$_u_count" -gt 0 ]; then
				u="%$_u_count${ZSH_VERSION+%}"
			fi
		fi
	...
	local f=""
	local _piece
	for _piece in "$h" "$w" "$i" "$s" "$u" "$p"; do
		[ -n "$_piece" ] && f="${f:+$f }$_piece"
	done
